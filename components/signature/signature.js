class Handwriting {
  ...
  selectSlideValue(slideValue) {
	 ...
	 * @param {string} angle 图片旋转角度
	 */
	rotateImg(url, angle){
		angle = angle || this.ANGLE['-90'];
		
		return new Promise((resolve, reject)=>{
			if(null == angle)return reject({msg:"this angle doesn't need to rotate!"});
			plus.zip.compressImage({
			    src: url,
				rotate: angle,
				dst: "_doc/" + Date.now() + '_' + parseInt(Math.random()*100) + ".png"
			  },function(e){
				  resolve(e);
			  },function(e){
				  reject(e);
