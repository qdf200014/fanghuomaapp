    ...
    <view class="vue-cropper" ref="cropper" :style="{ top : `${containerTop}px` }" v-show="show">
        <view class="cropper-box">
            <view class="cropper-box-canvas" @touchstart.stop.prevent="imgTouchStart" @touchmove.stop.prevent="imgMoveing" @touchend.stop.prevent="imgMoveEnd" :style="{
					...
					'transform': 'scale(' + imageScale + ',' + imageScale + ') ' + 'translate3d('+ imageX / imageScale + 'px,' + imageY / imageScale + 'px,' + '0)'
        ...
        <view class="cropper-crop-box" :class="{'pointer-events': cropFixed}" :style="{'width': cropW + 'px','height': cropH + 'px','transform': 'translate3d('+ cropOffsertX + 'px,' + cropOffsertY + 'px,' + '0)'}">
            <view class="cropper-view-box">
                <image :style="{'width': imageWidth + 'px','height': imageHeight + 'px','transform': 'scale(' + imageScale + ',' + imageScale + ') ' + 'translate3d('+ (imageX - cropOffsertX) / imageScale  + 'px,' + (imageY - cropOffsertY) / imageScale + 'px,' + '0)' + 'rotateZ('+ rotate * 90 +'deg)'}" mode="scaleToFill" :src="src" alt="cropper-img"></image>
            </view>
...
<script>
    export default {
        ...
        computed: {
            ...
            windowHeight() {
                return this.sysInfo.windowHeight - this.BTN_H;
            },
            // 图片宽高比
            imageRatio() {
			...
			// 图片缩放
			imageScale(){
				let scale = Math.max(this.cropHeight/this.imageHeight, this.cropWidth/this.imageWidth);
				scale =  Math.max(scale, this.scale);
				// let scale =  Math.max(0, this.scale);
				return scale;
			},
			// 图片位置
			imageX(){
				let x = this.x;
				let diff_x_l = this.imageWidth - this.cropWidth - this.IMG_PADDING;
				let diff_x_r = -1*diff_x_l;
				x = x>diff_x_l ? diff_x_l : x;
				x = x<diff_x_r ? diff_x_r : x;
				return x;
			},
			imageY(){
				let y = this.y;
				let diff_y_t = this.containerHeight / 2 - this.cropHeight / 2;
				let diff_y_b = diff_y_t - (this.imageHeight - this.cropHeight);
				y = y > diff_y_t ? diff_y_t : y;
				y = y < diff_y_b ? diff_y_b : y;
				return y;
			}
        },
        methods: {
            ...
            confirm(event) {
                ...
                const imgW = _this.imageWidth * _this.imageScale;
                const imgH = _this.imageHeight * _this.imageScale
            ...
            imgMoveing(e) {
                if(this.scaling) {
					...
					let minScale = Math.max(this.cropHeight/this.imageHeight, this.cropWidth/this.imageWidth);
