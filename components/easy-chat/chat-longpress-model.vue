		...
		<view class="im-bg-black-1 im-flex-column im-round-2 im-p-1 im-py-2 im-position-relative">
			...
			<view v-for="(item, index) in menuItem" :key="index" class="im-flex im-align-center">
				<view v-for="item2 in item" :key="item2.id" class="im-flex-column im-align-center im-mx-2 im-mt-1" @tap="onClick(item2, $event)">
					...
					<text class="im-font-23 im-font-white mt-1">{{item2.text}}</text>
				</view>
			</view>
		</view>
		...
		computed: {
			...
			menuItem() {
				...
				if (type === 'text') {
					arr.push({
						text: '复制',
						icon: '/static/easy-chat/chat/fuzhi.png',
						id: 'copy'
					})
					// arr.push({
					// 	text: '翻译',
					// 	icon: '/static/easy-chat/chat/fanyi.png',
					// 	id: 'translate'
					// })
				}
				...
				if (this.longpressMsg.flow === 'out') {
					arr.push({
						text: '撤回',
						icon: '/static/easy-chat/chat/chehui.png',
						id: 'withdraw'
					})
				}
				...
				arr.push({
					text: '删除',
					icon: '/static/easy-chat/chat/shanchu.png',
					id: 'delete'
				})
