<template>
	<view class="im-flex im-justify-between im-align-center im-ml-3 im-pr-3 im-py-2 im-border-bottom" @tap="onClick">
		<view class="im-flex im-align-center im-flex-1">
			...
			<text v-if="$attrs.session.isTop" class="im-font-28 im-font-blue-2 im-ml-1">{{ $attrs.session.isTop ? '[置顶]' : '' }}</text>
			<text class="im-font-32 im-text-ellipsis im-flex-1 im-ml-1">{{sessionName}}</text>
		</view>
