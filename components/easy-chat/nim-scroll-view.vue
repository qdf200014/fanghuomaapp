<template>
	<view class="im-flex-column im-flex-1" @touchend="clickScrollView">
		<scroll-view
			...
			ref="scrollView"
			class="im-flex-1"
			style="direction: rtl;max-height: 100%;transform:rotate(180deg);-ms-transform:rotate(180deg);-moz-transform:rotate(180deg);
		...
		>
			...
			<view>
				...
				<!-- loading加载 -->
				<view class="im-flex im-justify-center im-py-2 im-align-center im-w-100" style="height: 30px;">
					<text
						v-if="isLoadMore"
						class="im-font-28"
...
export default {
	...
	methods: {
		...
		loadMore() {
			...
			console.log('加载更多··········', );
			let nowIdClient = lastMsg && lastMsg.idClient || ''
			if (this.tempLastMsg && nowIdClient === this.tempLastMsg.idClient) {
				this.isLoadMore = false
				return ;
			}
			
			this.tempLastMsg = Object.assign({} , lastMsg)
			// 拉取云端的历史信息
			this.$store
