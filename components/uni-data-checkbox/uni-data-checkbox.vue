<template>
	<view class="uni-data-checklist">
		...
		<template v-else>
			<checkbox-group v-if="multiple" class="checklist-group" :class="{'is-list':mode==='list','is-wrap':wrap}" @change="chagne" style="padding-top: 32rpx;">
				<label class="checklist-box flex justify-start align-start" :class="item.labelClass" :style="[item.styleBackgroud]" v-for="(item,index) in dataList"
				 :key="index">
					<checkbox hidden :value="item.value+''" :checked="item.selected"/>
					<!-- #ifndef APP-NVUE -->
					...
					<view class="checklist-content" :class="{'list-content':mode === 'list' && icon ==='left'}">
						<text class="checklist-text" :class="item.textClass" :style="[item.styleIconText]" style="width: 560rpx;line-height: 40rpx;">{{item.text}}</text>
						<view v-if="mode === 'list' && icon === 'right'" class="checkobx__list" :class="item.listClass" :style="[item.styleBackgroud]"></view>
					</view>
				</label>
			</checkbox-group>
			<radio-group v-else class="checklist-group" :class="{'is-list':mode==='list','is-wrap':wrap}" @change.stop="chagne($event)">
				<label class="checklist-box" :class="item.labelClass" :style="[item.styleBackgroud]" v-for="(item,index) in dataList" :key="index">
					...
					<view class="checklist-content" :class="{'list-content':mode === 'list' && icon ==='left'}">
						...
						<text class="checklist-text" :class="item.textClass" :style="[item.styleIconText]">{{item.text1 || ''}}</text>
						<view v-if="mode === 'list' && icon === 'right'" class="checkobx__list" :class="item.listClass" :style="[item.styleRightIcon]"></view>
					</view>
				</label>
				<view class=" flex justify-center position-absolute top-0 right-0" v-for="(item,index) in dataList" v-if="item.cityType == 3" :key="index">
					<picker @change="bindPickerChange($event)" :value="index" :range="array">
						<text class="textSize">{{choseCity || '请选择'}}</text>
					</picker>
					<text class="pr-3 pl-1" style="font-size: 14px;color: transparent;">{{item.text1 || ''}}</text>
				</view>
			</radio-group>
		</template>
	</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				array: ['京','津','冀','晋','蒙','辽','吉','黑','沪','苏','浙','皖','闽','赣','鲁','豫','鄂','湘','粤','桂','琼','渝','川','黔','滇','藏','陕','甘','青','宁','新','台','港','澳','兵','大'],
				index: -1,
				dataList: [],
				range: [],
				contentText: {
		...
		created() {
			this.index = uni.getStorageSync('choseIndex')
			
			if(this.guoProvice){
				this.index = this.array.indexOf(this.guoProvice);
			}
		...
		methods: {
			bindPickerChange: function(event) {
				...
				this.$emit('getCity',this.choseCity)
				uni.setStorageSync('choseIndex',this.index)
			},
			init(range) {},
			/**
			 * 获取父元素实例
...
<style>
	...
	.choseCity{
		...
		height: 60rpx; 
		width: 200rpx; 
		border-bottom: 1rpx solid #323234;
