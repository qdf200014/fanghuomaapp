		...
		<template v-else>
			<checkbox-group v-if="multiple" class="checklist-group" :class="{'is-list':mode==='list','is-wrap':wrap}" @change="chagne">
				<label class="checklist-box" :class="item.labelClass" :style="[item.styleBackgroud]" v-for="(item,index) in dataList"
				 :key="index">
					<checkbox hidden :value="item.value+''" :checked="item.selected" :disabled="true"/>
					<!-- #ifndef APP-NVUE -->
					...
					<view class="checklist-content" :class="{'list-content':mode === 'list' && icon ==='left'}">
						<input type="text" style="width: 100rpx;" class="border-bottom-2">
			...
			<radio-group v-else class="checklist-group" :class="{'is-list':mode==='list','is-wrap':wrap}" @change="chagne">
				<label class="checklist-box" :class="item.labelClass" :style="[item.styleBackgroud]" v-for="(item,index) in dataList" :key="index">
					...
					<!-- #endif -->
					<view class="checklist-content" :class="{'list-content':mode === 'list' && icon ==='left'}">
						<radio hidden :disabled="item.disable" @click="dianbtn(item,$event)" :value="item.value+''" :checked="item.selected"/>
						<input type="text" style="width: 350rpx;text-align: center;font-size: 28rpx;" @input="changeInput(item, index)" class="border-bottom-3" :disabled="!item.selected" v-model="item.NText">
						<text class="checklist-text" :class="item.textClass" :style="[item.styleIconText]">{{item.text}}</text>
...
<script>
	...
	export default {
		...
		created() {
			console.log(this.localdata,'local打他')
