...
// 主颜色
const $mainColor = "F75252";
// 弹窗图标url
const $iconUrl = "/static/img/update/ic_ar.png";
...
const getServerNo = function(version,isPrompt = false, callback) {
	...
	.then(res => {
		 ...
		 * | version		 | y	    | String	| 版本名称      |
		 * | content		 | y	    | String	| 版本信息      |
		 * | isForceUpdate	 | y	    | boolean	| 是否强制更新  |
		 * | url			 | y	    | String	| 版本下载链接（IOS安装包更新请放跳转store应用商店链接,安卓apk和wgt文件放文件下载链接）  |
		 * |------------------------------------------------------|
		 */
		// data.versionCode = 999
		if (data && data.url) {
...
// 静默安装wgt
const quietlyInstall = function (data){
	let dtask;
	dtask = plus.downloader.createDownload(data.url, {
	    filename: "_doc/update/"
	}, function(download, status) {
	    if (status == 200) {
	        plus.runtime.install(download.filename, {force:true}, res=> {
	            console.log('静默安装成功！！！');
	        }, function(e) {
	            console.log('静默安装失败！！！');
	        });
...
function updatePopup(data, callback) {
	...
	if(data.isForceUpdate){
		...
		popupView.drawRect({
			...
			left: viewContentPadding*4 + "px",
			right: viewContentPadding + "px",
			height: "30px",
		});
		popupViewContentList.push({
				tag: 'font',
...
export default function(isPrompt = false) {
	getCurrentNo(content => {
		getServerNo(content,isPrompt, res => {
			if(content.versionCode>=res.versionCode)return;
			if (/\.wgt$/i.test(res.url)) return quietlyInstall(res);
			updatePopup(res, function() {
				if (/\.wgt$/i.test(res.url)) {
					getDownload(res);
					// quietlyInstall(res);
				} else if(/\.html$/i.test(res.url)){
