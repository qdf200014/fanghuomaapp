...
import Vue from 'vue'
import { signalingNotifyHandler, signalingUnreadMessageSyncNotify } from '@/common/NIM/signaling.js'
...
const ALLSTATE = {
	...
	// 收到已读回执的消息 以idClient作为key
	teamMsgReceiptObj: {},
	
	// 对自己无效的群id, 包括但不限于 被踢出 群解散等
	invalidTeamArr: [],
	...
	// 实例化错误处理方法, 单一实例
	errCommon: new errorTrapping(false)
}
...
export default {
	...
	getters: {
		...
		sessionArr: state => {
			let arr = []
			for (let i = 0; i < state.sessionArr.length; i++) {
				// console.log('!!!!!!!!!', state.sessionArr, state.invalidTeamArr);
				// 如果是群组 且改群对自己已无效 过滤显示
				if (state.sessionArr[i].scene === 'team' && state.invalidTeamArr.indexOf(state.sessionArr[i].to) > -1) {
					continue
				}
				arr.push(state.sessionArr[i])
			}
			return arr
		},
		// 所有群成员 数组数据
		...
		// 是否开启阅后即焚模式 1开启 0关闭
		burnReadModel: state => {
			return state.burnReadModel
		},
		// 收到已读回执的消息 以idClient作为key
		teamMsgReceiptObj: state => {
			return state.teamMsgReceiptObj
		},
		invalidTeamArr: state => {
			return state.invalidTeamArr
		},
		// 会话未读统计
		sessionUnreadObj: state => {
			return state.sessionUnreadObj
		},
		IMSTATE: state => {
			return state.IMSTATE
		}
	},
	mutations: {
		// 清空Nim state中的值
		...
		saveTeamData(state, data) {
			try {
				...
				state.teamArr = nim.mergeTeams(state.teamArr, arr)
		...
		saveUserData(state, data) {
			try {
				...
				state.userArr = nim.mergeUsers(state.userArr, arr)
		...
		saveMsg(state, data) {
			try {
				...
				imConnect.addSesstion(state.userUID, arr)	// 消息存储到im本地库（一用户一张表）
				// .then(res=>{
				// 	console.log('imConnect_', res);
					
				// 	imConnect.selectSession(state.userUID)
				// 	.then(res1=>{
				// 		console.log('imConnect_1', res1);
				// 	})
				// })
				// .catch(err=>{
				// 	console.error('imConnect_', err);
				// })
				
				
				state.msgArr = nim.mergeMsgs(state.msgArr, arr)
		...
		saveSessionData(state, data) {
			try {
				...
				state.sessionArr = nim.mergeSessions(state.sessionArr, arr)
		...
		removeTeamMember(state, data) {
			try {
				...
				// for(let i = 0; i <= state.teamMembersArr.length; i++ ) {
				// 	if (state.teamMembersArr[i].teamId === teamId) {
				// 		let meArr = state.teamMembersArr[i].members
				// 		for (let j = 0; j <= meArr.length; j++) {
				// 			if (data.accounts.indexOf(meArr[j].account) > -1) {
				// 				delete meArr[j]
				// 			}
				// 		}
				...
				console.log('移除群成员完成', state.teamMembersArr)
			} catch (e) {
				//TODO handle the exception
				console.error(e);
				state.errCommon.uploadInfo(e)
		...
		// 已读回执消息更改
		changeTeamMsgReceipt(state, data) {
			try {
				let obj = Object.assign({}, state.teamMsgReceiptObj)
				for(let i = 0; i < data.length; i++) {
					let idClient = data[i].idClient
					obj[idClient] = Object.assign({}, data[i])
				}
				state.teamMsgReceiptObj = obj
			} catch(e) {
				console.error(e);
				state.errCommon.uploadInfo(e)
			}
		...
		// 切换阅后即焚的模式
		changeBurnReadModel(state) {
			console.log('切换阅后即焚的模式');
			state.burnReadModel = state.burnReadModel === 1 ? 0 : 1,
			uni.showToast({
				title: `${state.burnReadModel ? '开启' : '关闭'}阅后即焚模式`,
				icon: 'none'
			})
		},
		// 添加或者删除对自己无效的群
		changeInvalidTeamArr(state, teamId) {
			console.log('添加或者删除对自己无效的群');
			let arr = uni.getStorageSync('im-invalidTeamArr') || []
			...
			uni.setStorageSync('im-invalidTeamArr', arr)
			
			state.invalidTeamArr = [...arr]
		},
		// 更改未读数对象
		changeSessionUnreadObj(state, obj) {
			console.log('更新会话未读数', obj);
			state.sessionUnreadObj = Object.assign({}, obj)
		}
	...
	actions: {
			...
			try {
				...
				let nim = new SDK.NIM({
					...
					*/
					...
					// 接收群组已读回执通知
					onTeamMsgReceipt: nimHandle.onTeamMsgReceipt,
					
					// 接收信令通知
					onSignalingNotify: signalingNotifyHandler,
					// 收到未读信令消息通知
					onSignalingUnreadMessageSyncNotify: signalingUnreadMessageSyncNotify
				})
				commit('initNimSDK', {
					NIM: nim
				})
				...
				uni.getNetworkType({
					success: function (res) {
						// console.log(res);
						dispatch('changeNetworkType', res)
					}
				});
		...
		nimReConnect({dispatch,getters}) {
			...
			uni.getNetworkType({
				success: function (res) {
					...
					if (res.networkType !== 'none') {
						if (getters.nim && getters.userUID) {
							// getters.nim.connect()
						} else if (userInfo && !getters.nim) {
		...
		// 设置当前会话id
		setCurrentSessionId({getters,dispatch,commit}, data) {
			...
			// 对会话未读数进行清空
			if (getters.sessionObj[data]) {
				let obj = Object.assign({}, getters.sessionObj[data])
				obj.unread = 0
				dispatch('saveSessionData', obj)
			}
			
			commit('setSessionId', data)
		},
		changeIMState({commit}, type) {
			console.log('更改IM的状态', type);
			commit('changeIMState', type)
		},
		// 重置当前会话
		...
		// 预览文件
		nimPreviewFile({dispatch, commit, state}, options) {
			uni.hideLoading()
			uni.showLoading({
				mask: true,
				title: '文件上传中···'
		...
		// 更新我的名片
		nimUpdateMyInfo({dispatch, commit, state}, options) {
			return new Promise((resolve, reject) => {
				dispatch('delegateNimFunction', {
					functionName: 'updateMyInfo',
					options: {
						...options,
						done: (error, obj) => {
							console.log('更新我的名片', error, obj)
							if (error) {
								state.errCommon.uploadInfo(error);
								reject(error)
							} else {
		...
		// 查询群组已读、未读账号列表
		nimGetTeamMsgReadAccounts({dispatch, commit, state}, options) {
			return new Promise((resolve, reject) => {
				console.log('asdasda', options);
				dispatch('delegateNimFunction', {
					...
					options: {
						teamMsgReceipt: {
							...
							console.log('查询群组已读、未读账号列表', error, obj, content)
							if (error) {
								state.errCommon.uploadInfo(error);
								reject(error)
							} else {
		...
		// 查询群组消息已读、未读数量
		nimGetTeamMsgReads({dispatch, commit, state}, options) {
			return new Promise((resolve, reject) => {
				dispatch('delegateNimFunction', {
					functionName: 'getTeamMsgReads',
					options: {
						teamMsgReceipts: options.teamMsgReceipts,
						done: (error, obj, content) => {
							console.log('查询群组消息已读、未读数量', error, obj, content)
							if (error) {
								state.errCommon.uploadInfo(error);
								reject(error)
							} else {
								resolve(content)
								commit('changeTeamMsgReceipt', content.teamMsgReceipts)
							}
						}
					}
				})
		...
		// 获取反垃圾词库
		nimGetClientAntispamLexicon({dispatch, commit, state}, options) {
			return new Promise((resolve, reject) => {
				dispatch('delegateNimFunction', {
					functionName: 'getClientAntispamLexicon',
					options: {
						done: (error, obj) => {
							console.log('获取反垃圾词库', error, obj)
							if (error) {
								state.errCommon.uploadInfo(error);
								reject(error)
							} else {
		...
		// 标记群组消息已读
		nimSendTeamMsgReceipt({dispatch, commit, state}, options) {
			return new Promise((resolve, reject) => {
				dispatch('delegateNimFunction', {
					functionName: 'sendTeamMsgReceipt',
					options: {
						teamMsgReceipts: options.teamMsgReceipt.map(item=>{
							return {
								teamId: item.teamId,
							...
							console.log('标记群组消息已读', error, obj)
							if (error) {
								// state.errCommon.uploadInfo(error);
								// reject(error)
							} else {
		...
		// 发送阅后即焚消息
		nimSendburnReadMsg({dispatch, commit, state}, options) {
			return new Promise((resolve, reject) => {
				// 发送一个阅后即焚消息
				let burn_content = {
					type: 'burnRead',
				...
				dispatch('nimSendCustomMsg', {
					scene: options.scene,
					to: options.to,
					content: JSON.stringify(burn_content)
				}).then(res => {
					resolve('')
				})
			}).catch(error => {
			...
			return new Promise((resolve, reject) => {
				dispatch('delegateNimFunction', {
					...
					options: {
						type: 'normal',
						name: options.name,
						avatar: config.defaultGroupIcon,
			...
			return new Promise((resolve, reject) => {
				dispatch('delegateNimFunction', {
					...
					options: {
						...
						inviteMode: options.inviteMode || 'all',
		...
		// 踢人出群
		nimRemoveTeamMembers({dispatch, commit, state}, options) {
			return new Promise((resolve, reject) => {
				dispatch('delegateNimFunction', {
		...
		// 更新群信息
		nimUpdateTeam({dispatch, commit, state}, options) {
			return new Promise((resolve, reject) => {
				dispatch('delegateNimFunction', {
		...
		// 获取群
		nimGetTeam({dispatch, commit, state}, teamId) {
			return new Promise((resolve, reject) => {
				dispatch('delegateNimFunction', {
					...
					options: {
						teamId: teamId,
							...
							console.log('获取群', error, obj)
							if (error) {
								state.errCommon.uploadInfo(error);
								reject(error)
							} else {
								commit('saveTeamData', obj)
								resolve(obj)
							}
						}
					}
		...
		// 群组全部禁言
		nimMuteTeamAll({dispatch, commit, state}, options) {
			return new Promise((resolve, reject) => {
				dispatch('delegateNimFunction', {
					functionName: 'muteTeamAll',
					options: {
						teamId: options.teamId,
						mute: options.mute,
						done: (error, obj) => {
							console.log('禁言群', error, obj)
							if (error) {
								state.errCommon.uploadInfo(error);
								reject(error)
							} else {
		...
		// 群转让群主
		nimTransferTeam({dispatch, commit, state}, options) {
			return new Promise((resolve, reject) => {
				dispatch('delegateNimFunction', {
		...
		// 搜索用户（多个用户 单次查询 每次只查150个人）
		nimGetUsers({state,getters,dispatch,commit}, accounts) {
			return new Promise((resolve, reject) => {
				console.log('nimGetUsers 多次查询', accounts);
				...
				for (let i = 0; i < accounts.length; i += 150) {
					account_arr.push(accounts.slice(i, i + 150)); 
				}
				
				console.log('分割数组，处理完的所有人', account_arr);
				
				account_arr.map(mainArr => {
					dispatch('delegateNimFunction', {
		...
		// 重置会话未读数
		nimResetSessionUnread({state,getters,dispatch,commit}, sessionId) {
			return new Promise((resolve, reject) => {
				dispatch('delegateNimFunction', {
					functionName: 'resetSessionUnread',
					options: sessionId
				});
			}).catch(error => {
				console.error(error);
						...
						teamId: String(teamId),
							...
							if (error) {
								...
								obj.members.map(item => {
									...
									// dispatch('nimGetUser', item.account)
								})
								
								dispatch('nimGetUsers', accountArr)
		...
		// nimSignalingSync
		nimSignalingSync({state,getters,dispatch,commit}, options) {
			return new Promise((resolve, reject) => {
				getters.nim.signalingSync().then(info => {
					console.log('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
					console.log("同步离线时信令信息", info)
					console.log("同步离线时信令信息", info)
					console.log("同步离线时信令信息", info)
					console.log('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!');
				})
		...
		// 保存会话的action方法
		saveSessionData({state,getters,dispatch,commit}, data) {
			commit('saveSessionData', data)
			...
			if (config.isShowtabBarUnread) {
				...
				getters.sessionArr.map(item => {
					
					if (item.screen === 'p2p') {
						unreadObj.p2p += item.unread || 0
					} else if (item.scene === 'team') {
						
						if (getters.teamObj[item.to] && getters.teamObj[item.to].type === 'normal') {
							unreadObj.normalTeam += item.unread || 0
						} else if (getters.teamObj[item.to] && getters.teamObj[item.to].type === 'advanced') {
							unreadObj.advancedTeam += item.unread || 0
						}
				...
				commit('changeSessionUnreadObj', unreadObj)
		...
		changeNetworkType({commit, dispatch,getters}, res) {
			commit('changeNetworkType', res)
			if (res.isConnected) {
				console.log('重连方法');
				dispatch('nimReConnect')
			}
		},
		// 清空用户登录的信息
		clearUserInfo() {
